...
def test_malformed_token_shape_rejected(monkeypatch, capsys, tmp_config, clean_env):
    ...
    save_config(
        {
            "OWA_REFRESH_TOKEN": "plain-chromium-session-token",
...
def test_decode_prints_synthetic_jwt(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
    ...
    save_config(
        {
            "OWA_REFRESH_TOKEN": "1.AQ_fake-rt-for-tests",
            "OWA_TENANT_ID": "00000000-0000-0000-0000-000000000000",
        }
    )

    token = make_jwt(
        {"exp": 9_999_999_999, "aud": "https://graph.microsoft.com", "scp": "Mail.Read Mail.Send"}
    )
    monkeypatch.setattr(
        "owa_piggy.token_flow.exchange_token",
        lambda rt, tid, cid, scope: {"access_token": token, "expires_in": 3600},
    )
...
def test_remaining_prints_minutes(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt, frozen_time
):
    from owa_piggy.config import save_config
    ...
    monkeypatch.setattr(
        "owa_piggy.token_flow.exchange_token",
        lambda *a, **k: {"access_token": token, "expires_in": 3600},
    )
...
def test_scope_without_value_errors(monkeypatch, capsys, tmp_config, clean_env):
...
def test_debug_unknown_audience_errors(monkeypatch, capsys, tmp_config, clean_env):
...
def test_raw_token_to_stdout(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
...
def test_agent_token_adds_json_default(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
    from owa_piggy.config import save_config
...
def test_env_mode_emits_shell_vars(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
    from owa_piggy.config import save_config
...
def test_bare_token_env_works(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
...
def test_cache_short_circuits_exchange(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
...
def test_cache_writes_on_exchange(monkeypatch, tmp_config, clean_env, make_jwt):
...
def test_cache_does_not_cross_tenant_boundary(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
    ...
    save_config({"OWA_REFRESH_TOKEN": "1.AQ_fake", "OWA_TENANT_ID": "tenant-B"})
    scope, _ = resolve_audience()
    # Cache an AT under tenant A while the active config is tenant B.
    store_token("tenant-A", CLIENT_ID, scope, "at-from-tenant-A", int(_time.time()) + 3600)
...
def test_setup_clears_cache(monkeypatch, tmp_config, clean_env, make_jwt):
    ...
    save_config({"OWA_REFRESH_TOKEN": "1.AQ_fake", "OWA_TENANT_ID": "tid"})
    scope, _ = resolve_audience()
    store_token("tid", CLIENT_ID, scope, "stale", int(_time.time()) + 3600)
    assert load_cache() != {}
    ...
    monkeypatch.setattr(
        "owa_piggy.token_flow.exchange_token",
        lambda *a, **k: {
            "access_token": make_jwt({"exp": int(_time.time()) + 3600}),
            "expires_in": 3600,
        },
    )
...
def test_reseed_clears_cache(monkeypatch, tmp_config, clean_env):
    ...
    scope, _ = resolve_audience()
    store_token("tid", CLIENT_ID, scope, "pre-reseed-at", int(_time.time()) + 3600)
    assert load_cache() != {}
...
def test_edge_launches_profile_sidecar(monkeypatch, capsys, tmp_config, clean_env):
...
def test_edge_reports_missing_edge_binary(monkeypatch, capsys, tmp_config, clean_env):
...
def test_json_served_from_cache_without_refresh_token(
    ...
    store_token("tid", CLIENT_ID, scope, cached_token, int(_time.time()) + 3600)

    fresh_token = make_jwt({"exp": int(_time.time()) + 3600, "iss": "fresh-from-aad"})
    called = {"n": 0}

    def _exchange(*a, **k):
        called["n"] += 1
        return {"access_token": fresh_token, "refresh_token": "1.AQ_rotated", "expires_in": 3600}
...
def test_cache_hit_env_mode(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
...
def test_cache_hit_decode_mode(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
    ...
    scope, _ = resolve_audience()
    token = make_jwt(
        {"exp": int(_time.time()) + 3600, "aud": "https://graph.microsoft.com", "scp": "Mail.Read"}
    )
    store_token("tid", CLIENT_ID, scope, token, int(_time.time()) + 3600)
...
def test_unknown_flag_mixed_with_known_is_rejected(monkeypatch, capsys, tmp_config, clean_env):
...
def test_scope_value_not_treated_as_unknown_flag(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt
):
...
def test_audiences_with_multiple_profiles_no_default(monkeypatch, capsys, tmp_config, clean_env):
...
def test_status_profile_label_on_stderr(monkeypatch, capsys, tmp_config, clean_env):
...
def test_status_without_profile_iterates_all(monkeypatch, capsys, tmp_config, clean_env):
...
def test_status_no_profiles_configured(monkeypatch, capsys, tmp_config, clean_env):
...
def test_cli_rejects_traversal_profile(monkeypatch, capsys, tmp_config, clean_env):
...
def test_cli_rejects_nested_profile(monkeypatch, capsys, tmp_config, clean_env):
    rc = _run(monkeypatch, ["setup", "--profile", "work/sub"])
    assert rc != 0
    assert "invalid profile alias" in capsys.readouterr().err


def test_profiles_set_default_rejects_bad_alias(monkeypatch, capsys, tmp_config, clean_env):
    rc = _run(monkeypatch, ["profiles", "set-default", "../escape"])
    assert rc != 0
    assert "invalid profile alias" in capsys.readouterr().err


def test_profiles_delete_rejects_bad_alias(monkeypatch, capsys, tmp_config, clean_env):
...
def _stub_picker_environment(monkeypatch, *, profiles, default, enabled=None, keys=()):
    ...
    monkeypatch.setattr(
        tui,
        "load_profiles_conf",
        lambda: {"OWA_DEFAULT_PROFILE": default, "OWA_PROFILES": list(enabled)},
    )
...
def test_dashboard_space_toggles_profile(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        tui, "disable_profile", lambda alias: calls.append(("disable", alias)) or (True, "")
    )
    monkeypatch.setattr(
        tui, "enable_profile", lambda alias: calls.append(("enable", alias)) or (True, "")
    )
...
def test_dashboard_enter_sets_default(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(tui, "set_default_profile", lambda alias: calls.append(alias) or (True, ""))
...
def test_cache_hit_remaining_mode(monkeypatch, capsys, tmp_config, clean_env, make_jwt):
...
def test_reseed_all_and_scheduled_mutually_exclusive(monkeypatch, capsys, tmp_config, clean_env):
    rc = _run(monkeypatch, ["reseed", "--all", "--scheduled"])
    assert rc != 0
    assert "--all and --scheduled are mutually exclusive" in capsys.readouterr().err


def test_reseed_scheduled_and_profile_mutually_exclusive(
    monkeypatch, capsys, tmp_config, clean_env
):
...
def test_reseed_scheduled_dispatches_to_do_reseed_scheduled(
    monkeypatch, capsys, tmp_config, clean_env
):
    called = []
    monkeypatch.setattr(cli_mod, "do_reseed_scheduled", lambda: called.append(True) or 0)
    rc = _run(monkeypatch, ["reseed", "--scheduled"])
    assert rc == 0
    assert called == [True]


def test_reseed_scheduled_json_envelope_scope(monkeypatch, capsys, tmp_config, clean_env):
...
def test_profiles_schedule_unknown_profile_fails(monkeypatch, capsys, tmp_config, clean_env):
    rc = _run(monkeypatch, ["profiles", "schedule", "ghost"])
    assert rc != 0
    assert "not found" in capsys.readouterr().err


def test_profiles_schedule_adds_to_registry(monkeypatch, capsys, tmp_config, clean_env):
...
def test_profiles_unschedule_removes_from_registry(monkeypatch, capsys, tmp_config, clean_env):
    from owa_piggy import launchd as launchd_mod
    from owa_piggy.config import (
        ensure_profile_registered,
        load_profiles_conf,
...
def _bound_profile(make_jwt):
    ...
    clients.save_client("default", clients.TEAMS_WEB_CLIENT_ID, refresh_token="1.AQ_teams")
    return clients.TEAMS_WEB_CLIENT_ID


def test_teams_audience_mints_under_the_bound_client(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt
):
...
def test_other_audiences_still_mint_under_the_foci_client(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt
):
    import time as _time
...
def test_bound_client_rotation_lands_in_clients_json(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt
):
    ...
    assert rc == 0
    capsys.readouterr()
    assert clients.load_clients("default")[teams_id]["refresh_token"] == "1.AQ_teams_v2"
    config, _ = load_config()
    assert config["OWA_REFRESH_TOKEN"] == "1.AQ_foci"


def test_bound_and_foci_tokens_do_not_share_a_cache_entry(
    monkeypatch, capsys, tmp_config, clean_env, make_jwt
):
