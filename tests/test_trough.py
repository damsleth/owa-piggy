...
def test_fetch_foci_picks_freshest_refresh(monkeypatch):
    ...
    monkeypatch.setattr(trough, '_http_get_json',
                         lambda url, *, timeout: rows)
...
def test_trough_setup_persists_token_and_ua(tmp_config, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda url, *, tenant=None, sub=None, timeout=10, limit=50: (
            '1.AQ_fake-rt', 'tid-abc',
            {'tid': 'tid-abc', 'sub': 'oid-1', 'src_host': 'login.x',
             'last_seen': 123, 'expires_in_at_capture': 4200,
             'total_candidates': 1, 'matched': 1, 'token_len': 13},
        ),
    )
    cfg = {}
    ok = setup_mod.interactive_setup(
        cfg, alias='trough-test',
...
def test_trough_setup_surfaces_fetch_failure(tmp_config, monkeypatch):
    ...
    cfg = {}
    ok = setup_mod.interactive_setup(
        cfg, alias='trough-fail', trough_url='http://bad:8765')
    assert ok is False
