...
def test_saving_one_client_preserves_the_others(profile):
    ...
    clients.save_client(profile, DEVOPS_CLIENT_ID, refresh_token='rt-ado',
...
def test_parse_spec_forms():
    ...
    assert (cid, url, err) == (DEVOPS_CLIENT_ID,
                               'https://dev.azure.com/o/p/_workitems', '')
...
def test_other_audiences_stay_on_the_foci_token(profile):
    ...
    assert clients.select_for_scope(
        profile, 'https://outlook.office.com/.default openid') == (None, None)
...
def test_overlay_swaps_client_token_and_origin_without_mutating_config(profile):
    clients.save_client(profile, TEAMS, refresh_token='rt-teams')
    config = {'OWA_REFRESH_TOKEN': 'rt-foci', 'OWA_TENANT_ID': 'tid',
              'OWA_EMAIL': 'a@b.c'}
...
def two_profiles(tmp_path, monkeypatch):
    ...
    _write_profile(root, 'me', OWA_REFRESH_TOKEN='1.foci',
                   OWA_TENANT_ID='tid-1', OWA_EMAIL='me@corp.example')
    _write_profile(root, 'me-teams', OWA_REFRESH_TOKEN='1.teams',
                   OWA_CLIENT_ID=TEAMS, OWA_TENANT_ID='tid-1',
...
def test_fold_ignores_profiles_of_a_different_identity(tmp_path, monkeypatch):
    ...
    _write_profile(root, 'a', OWA_REFRESH_TOKEN='1.a', OWA_TENANT_ID='tid-a',
                   OWA_EMAIL='a@corp.example')
    _write_profile(root, 'b-teams', OWA_REFRESH_TOKEN='1.b', OWA_CLIENT_ID=TEAMS,
                   OWA_TENANT_ID='tid-b', OWA_EMAIL='b@corp.example')
...
def test_a_plain_foci_profile_is_never_a_fold_candidate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(config_mod, 'ROOT_DIR', root)
    monkeypatch.setattr(config_mod, 'CONFIG_PATH', root / 'profiles' / 'a' / 'config')
    _write_profile(root, 'a', OWA_REFRESH_TOKEN='1.a', OWA_CLIENT_ID=FOCI_CLIENT_ID,
                   OWA_TENANT_ID='tid', OWA_EMAIL='a@corp.example')
    _write_profile(root, 'b', OWA_REFRESH_TOKEN='1.b', OWA_TENANT_ID='tid',
                   OWA_EMAIL='a@corp.example')
...
def test_fold_stamp_makes_the_scan_run_once(two_profiles):
    ...
    _write_profile(two_profiles, 'me-ado', OWA_REFRESH_TOKEN='1.ado',
                   OWA_CLIENT_ID=DEVOPS_CLIENT_ID, OWA_TENANT_ID='tid-1',
                   OWA_EMAIL='me@corp.example')
