...
def test_reseed_all_skips_when_registry_present_but_empty(
    ...
    monkeypatch.setattr(
        reseed_mod,
        "do_reseed",
        lambda alias: calls.append(alias) or 0,
    )
...
def test_reseed_all_keeps_legacy_fallback_when_registry_missing(monkeypatch, tmp_config, clean_env):
...
def test_reseed_scheduled_only_touches_scheduled_profiles(monkeypatch, tmp_config, clean_env):
...
def test_reseed_scheduled_empty_is_not_an_error(monkeypatch, tmp_config, clean_env, capsys):
...
def test_reseed_scheduled_skips_missing_profile_dir(monkeypatch, tmp_config, clean_env, capsys):
    ...
    assert rc == 0
    assert calls == ["work"]
    assert "skipping scheduled profile with no config on disk: ghost" in capsys.readouterr().err
...
def test_profile_cdp_port_is_stable_and_matches_shell_formula():
    ...
    assert reseed_mod._profile_cdp_port("work") != reseed_mod._profile_cdp_port("personal")
...
def test_capture_reseed_env_overrides_persisted_headless(monkeypatch, tmp_config, clean_env):
...
def test_capture_reseed_no_fallback_when_already_non_headless(monkeypatch, tmp_config, clean_env):
...
def test_headless_pref_default_and_env(monkeypatch):
    ...
    assert _pref({"OWA_CAPTURE_HEADLESS": "0"}, env="1", monkeypatch=monkeypatch) is True
...
def test_headless_pref_expires_after_a_day(monkeypatch):
    ...
    fresh = {"OWA_CAPTURE_HEADLESS": "0", "OWA_CAPTURE_HEADLESS_AT": _stamp_hours_ago(1)}
    stale = {"OWA_CAPTURE_HEADLESS": "0", "OWA_CAPTURE_HEADLESS_AT": _stamp_hours_ago(25)}
...
def test_expired_headless_zero_is_retried_and_cleared(monkeypatch, tmp_config, clean_env):
...
def test_fallback_stamps_the_headless_preference(monkeypatch, tmp_config, clean_env):
    calls, saved = _mock_capture_reseed(
        monkeypatch,
        [
    ...
    monkeypatch.setattr(reseed_mod.sys.stdin, "isatty", lambda: False, raising=False)
...
def test_interactive_rescue_does_not_credit_headless(monkeypatch, tmp_config, clean_env):
