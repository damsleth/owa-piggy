...
def _stub_launchd(monkeypatch, *, installed=False, run_rc=0):
    ...
    calls = []
    monkeypatch.setattr(profiles, "launchd_is_scheduled", lambda alias: installed)
    monkeypatch.setattr(
        profiles, "launchd_unschedule", lambda alias: calls.append((alias, False)) or run_rc
    )
    return calls


def test_delete_profile_removes_dir_and_unregisters(monkeypatch, tmp_config, clean_env):
    _make_profile_dir("work")
    ensure_profile_registered("work")
...
def test_delete_profile_uninstalls_launchd_when_present(monkeypatch, tmp_config, clean_env):
    _make_profile_dir("work")
    ensure_profile_registered("work")
...
def test_delete_profile_stops_when_launchd_unschedule_fails(monkeypatch, tmp_config, clean_env):
    _make_profile_dir("work")
    ensure_profile_registered("work")
...
def test_delete_profile_skips_promotion_when_disabled(monkeypatch, tmp_config, clean_env):
    _make_profile_dir("work")
    _make_profile_dir("personal")
    ensure_profile_registered("work")
    ensure_profile_registered("personal")
...
def test_delete_profile_returns_error_when_unregister_fails(monkeypatch, tmp_config, clean_env):
    _make_profile_dir("work")
    ensure_profile_registered("work")
    _stub_launchd(monkeypatch, installed=False)
    monkeypatch.setattr(
        profiles, "unregister_profile", lambda alias: (_ for _ in ()).throw(OSError("disk full"))
    )
