        ...
        if isinstance(parsed, dict) and parsed.get("refresh_token"):
            if expected_client_id:
                ...
                if got_client_id != expected_client_id:
                    log(
                        f"token response bound to client_id={got_client_id}, "
    ...
    try:
        ...
        try:
            ...
            browser.call(
                ...
                {
                    "windowId": wid,
                    "bounds": {"left": -32000, "top": -32000, "width": 500, "height": 375},
                },
            )
            browser.call(
    ...
    *,
    ...
    try:
        ...
        token_response = _capture_token_response(
            session, deadline=deadline, log=log, tick=tick, expected_client_id=expected_client_id
        )
    ...
    *,
    ...
    for client_id, entry in targets:
        ...
        if status != "ok" or not rt:
            failed.append(name)
            print(
                f"[{alias}] client {name}: capture returned {status!r}; keeping the previous token",
                file=sys.stderr,
            )
            if status == "reauth":
