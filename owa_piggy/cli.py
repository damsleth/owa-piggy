...
one-time setup (two paths):
  ...
  B. Manual paste (legacy MSAL cache, plaintext localStorage):
     ...
     3. Paste this snippet:
          ...
          const tid = it.realm || find('|idtoken|').split('|')[5]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_setup.add_argument(
        ...
        help="Google OAuth client secret. Honors GOOGLE_OAUTH_CLIENT_SECRET as a default.",
    )
    p_setup.add_argument(
        "--json",
        action="store_true",
...
def _mint_and_emit(args: argparse.Namespace, *, mode: str) -> int:
    ...
    result, info = exchange_fresh(
        config, scope, persist=persist, capture_stderr=True, token_sink=token_sink
    )
    if info["stderr_text"]:
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    setup_rc: int = create_profile(
        ...
        google_client_secret=(
            getattr(args, "google_client_secret", None)
