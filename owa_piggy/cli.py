...
def _build_parser():
    ...
    parser.add_argument("--version", "-v", action="version", version=f"owa-piggy {__version__}")

    sub = parser.add_subparsers(dest="command", metavar="<command>")

    p_token = sub.add_parser("token", help="print access token (default when no command given)")
    _add_common_options(p_token)
    p_token.add_argument("--json", action="store_true", help="print full token response as JSON")
    p_token.add_argument(
        "--env", action="store_true", help="print ACCESS_TOKEN= and EXPIRES_IN= lines"
    )
    ...
    p_status.add_argument(
        "--json", action="store_true", help="print token health as JSON without token values"
    )
    p_status.add_argument(
        "--verbose", "-v", action="store_true", help="include audience and scopes lines"
    )

    p_debug = sub.add_parser("debug", help="dump full setup diagnostics for one profile")
    ...
    p_reseed.add_argument("--json", action="store_true", help="emit action envelope on stdout")
    ...
    p_tui = sub.add_parser("tui", help="interactive token-health dashboard (profiles + freshness)")
    _add_common_options(p_tui)

    sub.add_parser("audiences", help="list all known FOCI-accessible audiences")

    sub.add_parser("install-owa-tools", help="install the companion owa-tools suite via Homebrew")
    ...
    p_list.add_argument(
        "--json", action="store_true", dest="profiles_list_json", help="print profiles as JSON"
    )

    p_sd = profiles_sub.add_parser("set-default", help="make <alias> the default profile")
    p_sd.add_argument("alias", metavar="<alias>")
    p_sd.add_argument("--json", action="store_true", help="emit action envelope on stdout")
    ...
    p_del = profiles_sub.add_parser("delete", help="remove a profile config + Edge sidecar dir")
    p_del.add_argument("alias", metavar="<alias>")
    p_del.add_argument(
        "--force", action="store_true", help="allow deleting the profile currently marked default"
    )
    p_del.add_argument(
        "--yes",
        action="store_true",
    ...
    p_del.add_argument("--json", action="store_true", help="emit action envelope on stdout")
    ...
    p_sched.add_argument("--json", action="store_true", help="emit action envelope on stdout")
    ...
    p_unsched.add_argument("--json", action="store_true", help="emit action envelope on stdout")
...
def _mint_and_emit(args, *, mode):
    ...
    if templated_audience_name(
        args.audience, args.scope, profile_default
        ...
        if derived:
            ...
            if persist:
                print(
                    f"NOTE: derived SharePoint tenant {derived!r} via Graph; "
                    f"saved as OWA_SHAREPOINT_TENANT.",
                    file=sys.stderr,
                )
        else:
    ...
    result, info = exchange_fresh(config, scope, persist=persist, capture_stderr=True)
    if info["stderr_text"]:
    ...
    if not info["rt_present"]:
        print(
            f"ERROR: OWA_REFRESH_TOKEN not set for profile {alias!r}. "
    ...
    if info["rt_present"] and not info["rt_shape_ok"]:
        ...
        print(
            "ERROR: OWA_REFRESH_TOKEN does not look like an AAD FOCI refresh "
            'token (expected "1.AQ..." or "0.AQ..."). Plain Chromium browsers '
            "store a session-bound token that AAD will not accept. Reseed "
            "from Microsoft Edge via `owa-piggy setup`.",
            file=sys.stderr,
        )
        return 1
    if not info["tid_present"]:
        print(
            f"ERROR: OWA_TENANT_ID not set for profile {alias!r}. "
            f"Run: owa-piggy setup --profile {alias}",
            file=sys.stderr,
        )
        return 1
    ...
    if info["rotated"] and not persist:
        print(
            ...
            "or run `owa-piggy setup` to persist.",
            file=sys.stderr,
        )
...
def _cmd_setup(args):
    ...
    if email and trough_url:
        print(
            "ERROR: --email and --from-trough are mutually exclusive (pick one capture source).",
            file=sys.stderr,
        )
        return 1
...
def _cmd_status(args):
    ...
    if getattr(args, "json", False):
        report = status_report(
            alias, audience=args.audience, scope=args.scope, sharepoint_tenant=sp_tenant
        )
...
def _cmd_profiles(args):
    ...
    as_json = bool(getattr(args, "json", False) or getattr(args, "profiles_list_json", False))
    if sub == "new":
    ...
    if as_json:
        ...
        emit_action(
            action_envelope(
                ...
                warnings=[
                    "Refresh tokens cached in keychain are not auto-purged; remove them manually if needed."  # noqa: E501
                ],
                duration_ms=(time.monotonic() - t0) * 1000.0,
            )
        )
...
def _run_with_modes(raw, agent, err_json):
    ...
    if text:
        try:
            ...
            print(
                "--agent requires JSON stdout; this command did not emit JSON "
                "(try the command's --json flag)",
                file=sys.stderr,
            )
            return 2
    else:
