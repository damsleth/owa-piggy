...
`owa-piggy tui` is the one-screen answer to *"are my tokens fresh, and if not, fix it"*: a profile list with single-key actions (toggle, set-default, schedule, add/delete, reseed, edge, capture mode, edit config) plus a live freshness column from a `status` probe of every profile — green `fresh 58m`, yellow `expiring 4m`, red with the fix hint when a profile needs a reseed. Reseeding (`r`/`R`), toggling, and adding re-probe automatically; `g` forces a refresh. A capture-mode column shows whether this profile's silent reseed runs `headless` or with a (minimized, offscreen) `visible` Edge window; `h` pins the other mode, and a leading `~` means nobody pinned it - the value is the default or the fallback's self-expiring preference. `c` opens the profile's config file in `$EDITOR` for everything the dashboard has no key for. Probing is network-bound, so the screen paints a `probing...` skeleton first, then fills in results. Bare `owa-piggy profiles` on a TTY opens this same dashboard — `tui` is just the explicit, discoverable name for it. For scripts and CI, use `profiles list` (offline, no probe) or `status --json`.
